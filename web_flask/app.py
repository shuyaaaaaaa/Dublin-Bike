...
def index():
    try:
        ...
        wf=requests.get(login.owFor, params={'lat':login.owLat, 'lon':login.owLon, 'units':'metric','appid':login.owKey})
...
def detailed():
    ...
    if request.method == 'POST':
        try:
            ...
            # Collect the bike data from the database for the selected station and the current day
            try:
                cursor = conn.cursor()
            ...
            try:
                # Calculate the total number of occurences of that day
                total_days = set()
                for row in data:
                ...
                # Calculate the average for each hour of the current day
...
def route():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Identify closest station to start and end. Return updated route data with 'stops'
